...
.. seealso::
   For implementation-level discussion of the polynomial backend --
   including the ``shortcircuit``, ``atol``, and ``pack`` knobs of
   :meth:`~algebraic.polynomials.RankDecomposition.compose`, the
   trade-offs between the fast and smart prune paths, and the
   vectorization strategy used by ``algebraic.utils.poly`` -- see
   :doc:`../developer-notes`.
